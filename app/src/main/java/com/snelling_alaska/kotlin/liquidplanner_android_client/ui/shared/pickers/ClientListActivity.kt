...
class ClientListActivity : SharedPickerBase() {
  var adapter: ClientListAdapter? = null

  override fun getAdapter(): RecyclerView.Adapter<RecyclerView.ViewHolder> {
    val items = Current.space?.clients ?: listOf()

    return ClientListAdapter(items, this)
      .apply { adapter = this } as RecyclerView.Adapter<RecyclerView.ViewHolder>
  }

  fun onListFragmentInteraction(activity: Client) {
    val result = getReturnIntent()
    result.putExtra(SELECTED_CLIENT, activity)
  ...
  inner class ClientListAdapter(
    var items: List<Client>? = listOf(),
    var listener: ClientListActivity
  ...
  companion object {
    val SELECTED_CLIENT = "SELECTED_CLIENT"
  }
}